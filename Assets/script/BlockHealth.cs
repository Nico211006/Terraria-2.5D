...
public class BlockHealth : MonoBehaviour
{
    ...
    public void TakeDamage(int damage)
    {
        ...
        Debug.Log(gameObject.name + " hat Schaden bekommen. Leben: " + currentHealth);
    ...
    {
        Debug.Log("Block zerstört");

        if (blockDropPrefab != null)
        {
            Vector3 spawnPosition = transform.position + dropOffset;
            GameObject drop = Instantiate(blockDropPrefab, spawnPosition, Quaternion.identity);
            Debug.Log("Drop wurde erstellt: " + drop.name + " bei " + spawnPosition);
        }
        else
        {
            Debug.LogError("Kein blockDropPrefab im BlockHealth eingetragen!");
        }
